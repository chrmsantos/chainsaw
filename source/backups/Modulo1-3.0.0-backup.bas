...
'================================================================================
Private Const CHAINSAW_VERSION As String = "3.0.0"
...
Private Function SaveDocumentSafely(doc As Document) As Boolean
    On Error GoTo ErrorHandler
...
Private Function CopyEmentaToClipboard(doc As Document) As Boolean
    On Error GoTo ErrorHandler
...
Private Function PutTextInClipboard(text As String) As Boolean
    On Error GoTo ErrorHandler
...
Private Function PreviousChecking(doc As Document) As Boolean
    ...
    ' Verifica consistencia de enderecos entre 2 e 3 paragrafos
...
'================================================================================
Private Sub RemoveEmentaLeadingLabelPrefix(doc As Document)
    On Error GoTo ErrorHandler
...
Private Function CountValidCPFInText(text As String) As Long
    On Error GoTo ErrorHandler
...
Private Function CountValidCNPJInText(text As String) As Long
    On Error GoTo ErrorHandler
...
Private Function CountLikelyCreditCardsInText(text As String) As Long
    On Error GoTo ErrorHandler
...
Private Function CountCID10InText(text As String) As Long
    On Error GoTo ErrorHandler
    CountCID10InText = 0
...
Private Function OnlyDigits(text As String) As String
    Dim i As Long
    Dim ch As String
...
Private Function IsAllSameDigit(digits As String) As Boolean
    Dim i As Long
...
Private Function IsValidCPF(cpfDigits As String) As Boolean
    On Error GoTo ErrorHandler
    IsValidCPF = False
...
Private Function IsValidCNPJ(cnpjDigits As String) As Boolean
    On Error GoTo ErrorHandler
    IsValidCNPJ = False
...
Private Function IsLuhnValid(digits As String) As Boolean
    On Error GoTo ErrorHandler
    IsLuhnValid = False
...
Private Function FormatDocumentTitle(doc As Document) As Boolean
    ...
    If UBound(words) >= 0 Then
        ...
        If firstWord = "indicacao" Or firstWord = "requerimento" Or firstWord = "mocao" Then
            isProposition = True
        End If
    End If

    ' Se for proposicao, substitui a ultima palavra por $NUMERO$/$ANO$
    If isProposition And UBound(words) > 0 Then ' FIX: Changed >= 0 to > 0
        ' Reconstroi o texto substituindo a ultima palavra com validacao
        newText = ""
        ...
        ' Adiciona $NUMERO$/$ANO$ no lugar da ultima palavra
...
Private Function ProtectImagesInRange(targetRange As Range) As Boolean
    ...
    If targetRange.InlineShapes.count > 0 Then
        ' OTIMIZADO: Aplica formatacao caractere por caractere, protegendo imagens
        Dim i As Long
        Dim charRange As Range
        Dim charCount As Long
        ...
        If charCount > 0 Then ' Verificacao de seguranca
            For i = 1 To charCount
                ...
                ' So formata caracteres que nao sao parte de imagens
                If charRange.InlineShapes.count = 0 Then
                    With charRange.Font
                        .Name = STANDARD_FONT
                        .size = STANDARD_FONT_SIZE
        ...
        If Len(para.Range.text) <= 80 Then
            ...
            For Each term In terms
                ...
                If matchCount >= 2 Then
                    ...
                    para.Range.Select
                    Selection.MoveEnd unit:=wdCharacter, count:=-1 ' Exclui o marcador de paragrafo
                    Selection.Delete
                    Selection.InsertAfter plenarioAcento & " ""Dr. Tancredo Neves"", $DATAATUALEXTENSO$."
