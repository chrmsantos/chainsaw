...
Public Function ApplyTextReplacements(doc As Document) As Boolean
	...
	For i = 0 To UBound(dOesteVariants)
		With rng.Find: .ClearFormatting: .Replacement.ClearFormatting
			.Text = dOesteVariants(i) & "este": .Replacement.Text = "d'Oeste": .Forward = True: .Wrap = wdFindStop
			.Format = False: .MatchCase = False: .MatchWholeWord = False: .MatchWildcards = False
			Do While .Execute(Replace:=wdReplaceOne): replacementCount = replacementCount + 1: rng.Collapse wdCollapseEnd: Loop
		End With
	Next i
	Set rng = doc.Range
	...
	For i = 0 To UBound(dashVariants)
		If dashVariants(i) <> " — " Then
			With rng.Find: .ClearFormatting: .Replacement.ClearFormatting
				.Text = dashVariants(i): .Replacement.Text = " — ": .Forward = True: .Wrap = wdFindStop
				Do While .Execute(Replace:=wdReplaceOne): replacementCount = replacementCount + 1: rng.Collapse wdCollapseEnd: Loop
			End With
		End If
	Next i
	...
	For i = 0 To UBound(lineStartDashVariants)
		With rng.Find: .ClearFormatting: .Replacement.ClearFormatting
			.Text = lineStartDashVariants(i): .Replacement.Text = "^p— ": .Forward = True: .Wrap = wdFindStop
			Do While .Execute(Replace:=wdReplaceOne): replacementCount = replacementCount + 1: rng.Collapse wdCollapseEnd: Loop
		End With
	Next i
	Set rng = doc.Range
	...
	For i = 0 To UBound(lineEndDashVariants)
		With rng.Find: .ClearFormatting: .Replacement.ClearFormatting
			.Text = lineEndDashVariants(i): .Replacement.Text = " —^p": .Forward = True: .Wrap = wdFindStop
			Do While .Execute(Replace:=wdReplaceOne): replacementCount = replacementCount + 1: rng.Collapse wdCollapseEnd: Loop
		End With
	Next i
	Set rng = doc.Range
	With rng.Find: .ClearFormatting: .Replacement.ClearFormatting
		.Text = "^l": .Replacement.Text = " ": .Forward = True: .Wrap = wdFindStop
		Do While .Execute(Replace:=wdReplaceOne): replacementCount = replacementCount + 1: rng.Collapse wdCollapseEnd: Loop
	End With
	On Error Resume Next: Set rng = doc.Range
	With rng.Find: .ClearFormatting: .Replacement.ClearFormatting
		.Text = Chr(11): .Replacement.Text = " ": .Forward = True: .Wrap = wdFindStop
		Do While .Execute(Replace:=wdReplaceOne): If Err.Number <> 0 Then Exit Do: replacementCount = replacementCount + 1: rng.Collapse wdCollapseEnd: Loop
	End With: If Err.Number <> 0 Then Err.Clear: On Error GoTo ErrHandler
	On Error Resume Next: Set rng = doc.Range
	With rng.Find: .ClearFormatting: .Replacement.ClearFormatting
		.Text = Chr(10): .Replacement.Text = " ": .Forward = True: .Wrap = wdFindStop
		Do While .Execute(Replace:=wdReplaceOne): If Err.Number <> 0 Then Exit Do: replacementCount = replacementCount + 1: rng.Collapse wdCollapseEnd: Loop
	End With: If Err.Number <> 0 Then Err.Clear: On Error GoTo ErrHandler
...
Public Function ApplySpecificParagraphReplacements(doc As Document) As Boolean
	...
	If secondParaIndex > 0 Then
		...
		Do While idxStart <= Len(paraText) And (Mid$(paraText, idxStart, 1) = " " Or Mid$(paraText, idxStart, 1) = vbTab): idxStart = idxStart + 1: Loop
	...
	With rng.Find: .ClearFormatting: .Replacement.ClearFormatting
		...
		.MatchCase = True: .Wrap = wdFindContinue
		Do While .Execute(Replace:=wdReplaceOne): replacementCount = replacementCount + 1: rng.Collapse wdCollapseEnd: Loop
	End With
	...
	For j = 0 To UBound(wordsToUppercase)
		...
		With rng.Find: .ClearFormatting: .Replacement.ClearFormatting
			...
			.MatchCase = True: .MatchWholeWord = True: .Wrap = wdFindContinue
			Do While .Execute(Replace:=wdReplaceOne): replacementCount = replacementCount + 1: rng.Collapse wdCollapseEnd: Loop
		End With
	Next j
